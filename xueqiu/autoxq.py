...
# userid = input('Enter Xueqiu Account id as seedid or Enter q to quit:')
seedid = 2818960454
max_num = 500
num = max_num
...
while num:
    ...
    print(id)
    url = 'https://xueqiu.com/friendships/groups/members.json?page=1&uid=%s&gid=0' % (
        userid)
    ...
    for u in js['users']:
        ...
        try:
            ...
            cur.execute(
                'INSERT OR IGNORE INTO People (uid, name, retrieved) VALUES (?, ?, 0)', (user_id, user_name))
            conn.commit()
            if cur.rowcount != 1:
                print('***********')
