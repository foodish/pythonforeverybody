...
def get_page(uid, page):
    url = 'https://xueqiu.com/friendships/groups/members.json?page=%d&uid=%s&gid=0' % (
        page, uid)
...
def parse_page(js):
    ...
    for u in js['users']:
        ...
        try:
            ...
            cur.execute('INSERT OR IGNORE INTO People (uid, name, gender, province, fr_num, fo_num, retrieved) VALUES (?, ?, ?, ?, ?, ?, 0)', (u[
                        'id'], u['screen_name'], u['gender'], u['province'], u['friends_count'], u['followers_count']))
            conn.commit()
            if cur.rowcount != 1:
...
while True:
    ...
    if userid == 'q':
        ...
        try:
            ...
            cur.execute(
                '''INSERT OR IGNORE INTO People (uid, retrieved) VALUES (?, 0)''', (userid, ))
            conn.commit()
            if cur.rowcount != 1:
                print('___________')
