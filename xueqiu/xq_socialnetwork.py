...
def save_graph_nets(fans_level):
    ...
    cur.execute('select from_id, to_id from Follows where from_id in (select id from People where fo_num > ?)', (fans_level,))
...
def centrality():
    ...
    print('\ntop 10 in_degree of graph:\n')    
    for i in range(10):
        id, fans_num = sorted_in_degree[i]
        cur.execute('select name, fo_num from People where id = ?', (id,))
        name, fo_num = cur.fetchone()
    ...
    sorted_betweeness = sorted(betweenness.items(), key=lambda item:item[1], reverse=True)
